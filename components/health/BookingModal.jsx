...
export default function BookingModal({ visible, onClose, specialty }) {
  ...
  const handleConfirm = () => {
    if (!name.trim()) {
      error('Name Required', 'Please enter the patient name.');

      return;
    }

    if (!phone.trim()) {
      error('Phone Required', 'Please enter the mobile number.');
    ...
    if (!age.trim()) {
      error('Age Required', 'Please enter the patient age.');
    ...
    if (!gender) {
      error('Gender Required', 'Please select gender.');
